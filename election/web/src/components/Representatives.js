  ...
  const repsSearch = () => {
    if (address)
      setLoading(
        true,
        axios
          .get(`${API_ROOT_URL}/reps?address=${address}`)
          .then((response) => {
    ...
    <React.Fragment>
      ...
      <div className="Voter">
        ...
        <div className="Reps">
          {repsData &&
            repsData.officials &&
            repsData.officials.map((rep, index) => {
              return (
                <div
                  key={index}
                ...
                >
                  <img width="20%" src={rep.photoUrl} alt=""></img>
                  <br />
                  <span>{rep.name} </span>
                  <br />
                  <span className="link">{rep.party}</span>
                  <br />
                  {rep.phones &&
                    rep.phones.map((p, index) => {
                      return (
                        <span key={index}>
                          {" "}
                          <Icon.Phone size={18} />
                  ...
                  {rep.address &&
                    rep.address.map((a, index) => {
                      return (
                        <span key={index}>
                          <Icon.Briefcase size={18} /> {a.line1}
                          <br />
                    ...
                    rep.channels.map((channel, i) => {
                      return (
                        <span key={i}>
                          ...
                          <a
                            href={`${socialMediaUrls[channel.type]}${
                              channel.id
                            }`}
