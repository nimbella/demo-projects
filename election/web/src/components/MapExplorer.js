...
function MapExplorer({
  ...
  const hoveredRegion = useMemo(() => {
    const hoveredData =
      (regionHighlighted.countyName
        ? data[regionHighlighted.stateCode]?.counties?.[
            regionHighlighted.countyName
          ]
  ...
  }, [data, regionHighlighted.stateCode, regionHighlighted.countyName]);
  ...
  const handleCountyClick = useCallback(() => {
    const newMapView =
      mapView === MAP_VIEWS.COUNTIES ? MAP_VIEWS.STATES : MAP_VIEWS.COUNTIES;
      ...
      <div className="panel" ref={panelRef}>
        <div className="panel-left fadeInUp" style={trail[0]}>
          <h2 className={classnames(mapStatistic)}>
            {hoveredRegion.name}
            {hoveredRegion.name === UNKNOWN_COUNTY_KEY &&
              ` [${STATE_NAMES[regionHighlighted.stateCode]}]`}
          </h2>
          ...
          {regionHighlighted.stateCode && (
            <h1 className={classnames('county', mapStatistic)}>
              ...
              <span>{capitalize(statisticConfig.displayName)}</span>
            </h1>
          )}
        </div>
